...
public class Yahtzee2 extends GraphicsProgram implements YahtzeeConstants {

	public static void main(String[] args) {
		new Yahtzee2().start(args);
	}
	...
	private void updateScoreboard(int player) {
		...
		while (true) {
			...
			if (categories[player - 1][category - 1] == 0) { // checking if category is empty
				...
				break;
			} else if (category == YAHTZEE && categories[player - 1][category - 1] == 1) { // bonus on 1+ yahtzee
				int n = scoreCard[player - 1][category - 1] += 100;
				display.updateScorecard(category, player, n);
				break;
			}
	...
	private int getScore(boolean correctCategory, int player) {
		int score = 0;
